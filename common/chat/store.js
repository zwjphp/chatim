...
export default new Vuex.Store({
	...
	mutations: {
				...
				state.socketTask.onMessage((res) => {
					...
					switch (res.cmd) {
						case 10:
							...
							break;
						case 11:
							var key = "groupChat";// 群聊
							uni.$emit('chat_Group_list_add', {
								dstid: res.dstid
							});
							break;
						case 9:
							uni.$emit('FriendsList'); // 通知好友页，有新好友添加
							return;
						case 3:
							var key = "chartPage"; //单聊 响应页面
							// 敏感词
							uni.showToast({
								title: '存在敏感信息，发送失败',
								duration: 2000
		...
		webSocketReconnect(state) {
			...
			// 每过 5 秒尝试一次，检查是否连接成功， 直到超过最大连接次数
